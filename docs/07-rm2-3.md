...
# RM2 Visualisation, power, and effect size
...
In this pre-class, you won't be asked to write any code yourself, we will give you all the example code. Instead, play with the arguments, change TRUE to FALSE and vice-versa, change the values and colours etc.. This will help you learn what each bit does. 
...
* Download `Zhang et al. 2014 Study 3.csv` from Moodle and save it in your Lab 3 folder. Make sure that you do not change the file name at all.
...
The code below will create a simple histogram with default appearance and no customisation. You wouldn't use this graph in a paper, but if you just wanting to very quickly check your distributions, for e.g., normality, this code might be enough. 
...
Adding `binwdith` to `geom_histogram()` changes the bins of the histogram, i.e., how wide the bars are. The default is 30. Sometimes this may be appropriate but often you will want to change the binwidth. What value you give will depend upon your data.
...
It's often useful to add a regression line or line of best fit to a scatterplot. The regression line is added with `geom_smooth()` and by default will also provide a 95% confidence interval. 
...
For this example, we'll use gender and speaker as IVs. `fill()` now specifies a second IV, rather than repeating the IV on the x-axis as in the previous plot. 
...
<div class="figure" style="text-align: center">
<img src="07-rm2-3_files/figure-html/unnamed-chunk-3-1.png" alt="**CAPTION THIS FIGURE!!**" width="100%" />
<p class="caption">(\#fig:unnamed-chunk-3)**CAPTION THIS FIGURE!!**</p>
</div>

### Activity 5: Reordering factors

R orders categorical variables alphabetically. For Gender this didn't make much of a difference and for time 1 and 2 this make sense for them to be in this order but we may want to change the order of Condition (in my mind it makes more sense for Ordinary to come first, but that may just be me).
...
Because the functions we need don't exist we need to create them. Copy and paste all of the below code without changing anything. You do not need to understand this code. I don't. When you run this, you should see `geom_split_violin` appear in the Environment pane under Functions. 
...
Today we will use the function `pwr.t.test()`, `pwr.r.test()` and `pwr.chisq.test` from the package `pwr` to run power calculations for t-tests, correlations and chi-square.
...
* **w** - Effect size (you can enter Cramer's V which we calculated in RM1 Lab 3)
